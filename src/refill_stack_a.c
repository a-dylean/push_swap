...
void ft_push_to_stack_a(t_node **stack_a, t_node **stack_b)
{
	...
	{
		last_node_a = ft_get_last_node(*stack_a);
		max_node_b = ft_find_max_node(*stack_b);
		if (last_node_a->num < (*stack_a)->num && last_node_a->num > max_node_b.num)
		{
			temp_nodes -= 1;
			ft_reverse_rotate(stack_a, 'a');
		}	
		else if ((*stack_b)->num == max_node_b.num)
			ft_push(stack_b, stack_a, 'a');
		else if (temp_nodes == 0 || (*stack_b)->num > last_node_a->num)
		{
			temp_nodes += 1;
			ft_push(stack_b, stack_a, 'a');
			ft_rotate(stack_a, 'a');
			...
			ft_push_max_node(stack_a, stack_b, max_node_b);
	}
}
